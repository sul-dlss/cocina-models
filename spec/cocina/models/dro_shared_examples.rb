  ...
  describe '.from_dynamic' do
    ...
    context 'with empty subschemas' do
      ...
      it 'populates attribute subschemas with default values' do
        ...
        expect(instance.structural).to be_kind_of struct_class
        expect(instance.structural.hasAgreement).to eq '' if struct_class == Cocina::Models::DRO
      end
    end
  end
  ...
  describe '.from_json' do
    ...
    context 'with all specifiable properties' do
        ...
        <<~JSON
          {
            ...
            "type": "#{item_type}",
            "label": "#{required_properties[:label]}",
            "version": "#{required_properties[:version]}",
            "access": {
              "access": "dark",
              "embargo": {
                "releaseDate": "2009-12-14T07:00:00Z"
      ...
      it 'populates attributes passed in as expected' do
        ...
        embargo_attributes = instance.access.embargo.attributes
        expect(embargo_attributes).to eq(releaseDate: DateTime.parse('2009-12-14T07:00:00Z'),
                                         access: 'dark')
        ...
        expect(instance.identification.sourceId).to eq 'source:9999'
        link = instance.identification.catalogLinks.first
        expect(link.catalog).to eq 'symphony'
        expect(link.catalogRecordId).to eq '44444'

        expect(instance.structural.contains).to all(be_instance_of(struct_contains_class))
        expect(instance.structural.hasAgreement).to eq 'druid:666' if struct_class == Cocina::Models::DRO
        expect(instance.structural.isMemberOf).to eq 'druid:bc777df7777'
        expect(instance.structural.hasMemberOrders.first.viewingDirection).to eq 'left-to-right'
