  ...
  module Models
    class Embargo < Struct
      ...
      def method_missing(method_name, *arguments, &block)
        if %i[controlledDigitalLending releaseDate useAndReproductionStatement].include?(method_name)
          Deprecation.warn(
            self,
      ...
      def respond_to_missing?(method_name, include_private = false)
        %i[controlledDigitalLending releaseDate useAndReproductionStatement].include?(method_name) || super
      end
    end
  end
end
