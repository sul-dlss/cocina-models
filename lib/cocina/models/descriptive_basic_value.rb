  ...
  module Models
    class DescriptiveBasicValue < Struct
      ...
      def method_missing(method_name, *arguments, &block)
        if %i[structuredValue parallelValue groupedValue displayLabel valueLanguage
              valueAt].include?(method_name)
          Deprecation.warn(
            self,
      ...
      def respond_to_missing?(method_name, include_private = false)
        %i[structuredValue parallelValue groupedValue displayLabel valueLanguage
           valueAt].include?(method_name) || super
      end
    end
  end
end
